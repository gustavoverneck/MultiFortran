...
c----------------------------------------------------------------
      pi=dacos(-1.d0)
      pi2=pi*pi
      hc=197.326d0		!conversao 197.326 mev.fm = 3.16151093e-17 j.m
c-------------------------------------------------------
c     gm3 set of parameters (prc55,1997,540)
c--------------------------------------------------------
c      m=939.0d0
c      denss=0.153d0
c      ml(1)=0.511d0/m
...
c      rc=-0.002421d0
c      rxi=0.0d-9
c      write(6,*)'gm3', gs,gv,gr,rb,rc,rxi
c---------------------------------------------------
c     gm1 set of parameters   (k=300 mev m*=0.7m)   
c----------------------------------------------------
      m = 938.9187137765d0    	!massa nucleon - proton e neutron  
      denss=0.153d0		!Densidade de saturação nuclear n0 = 0.153 fm^−3
c	massas dos leptons
      ml(1)=0.511d0/m		!massa DO elétron por massa proton/neutron
      ml(2)=105.66d0/m		!massa DO muon por massa proton/neutron

c	massas dos barions
      mb(1) = 939.56534623d0/m	!massa do neutron por nucleon
      mb(2) = 938.272081323d0/m	!massa do proton por nucleon      
      mb(3) = 1116.d0/m 	!massa do lambda0 por massa proton/neutron   
      mb(4) = 1193.d0/m 	!massa do sigma- por massa proton/neutron  
      mb(5) = 1193.d0/m 	!massa do sigma0 por massa proton/neutron  
      mb(6) = 1193.d0/m 	!massa do sigma+ por massa proton/neutron
      mb(7) = 1318.d0/m 	!massa do xi- por massa proton/neutron
      mb(8) = 1318.d0/m 	!massa do xi0 por massa proton/neutron

c	massas dos mesons mediadores
      ms=400.d0/m		!massa DO sigma por massa proton/neutron
      mv=783.d0/m		!massa DO omega por massa proton/neutron
      mrho=770.d0/m		!massa DO rho por massa proton/neutron
c      xls=11.785d0*(m/hc)**2 		gsn^2/ms^2
c      xlw=7.148d0*(m/hc)**2		gwn^2/mw^2
c      xlr=4.410d0*(m/hc)**2    	grn^2/mr^2
      xs = 0.7d0
      xv = 0.783d0

      gs=dsqrt(11.785d0)/hc*m	!constante de acoplamento sigma convertida x massa proton
      gv=dsqrt(7.148d0)/hc*m	!constante de acoplamento omega convertida x massa proton
      gr=dsqrt(4.41d0)/hc*m	!constante de acoplamento rho convertida x massa proton
      rb=0.002948d0!*gs**3*2.d0 !0.005896d0*gs**3
      rc=-0.001071d0!*gs**4 !-0.006426*gs**4
      rxi=0.d0
c      write(6,*)'gm1', gs,gv,gr,rb,rc,rxi
c-------------------------------------------------------
c    tm1 set of parameters
c--------------------------------------------------------
!           m=938.0d0
!           denss=0.145d0
!           ml(1)=0.511d0/m
!           ml(2)=105.66d0/m
!           gs=10.0289d0/511.198d0*m
!           gv=12.6139d0/783.0d0*m
!           gr=9.2644d0/770.d0*m
!           rb=7.2325d0*197.326d0/m/(10.0289d0)**3
!           rc=0.6183d0/(10.0289d0)**4
!           rxi=71.3075d0/(12.6139d0)**4
c---------------------------------------------------------------------
c      B(n) = B_surf + B0{1 - exp[-beta(n/n0)^alpha]}
c---------------------------------------------------------------------

      b_star=3.3d6                     	! dimensionless field 
      ...
      b_cp=(1.0d0/ml(1))**2*b_ce   	! critical magnetic field  
c      b=b_star*b_ce			!campo em termos DO campo admensional
...
c      bg= 1.0d19
      ...
      b=b0*bce
      btsl = bg*1.0d-4
      bsurf = 1.0d11		!B sup é o campo na superfı́cie

c      bsurf = 1.0d13		!B sup é o campo na superfı́cie

c      betaa = 6.5d-3		!paramentro da eq 
c      alphaa = 3.50d0		!paramentro da eq 
c	write(*,*) 'campo local', b
      betaa = 1.0d-2		!paramentro da eq  6.32 
      alphaa = 3.0d0		!paramentro da eq  6.32 


      muninf=0.92d0		!potencial quim (k^2+ms^2)^1/2   [fm^-3] range limite inferior
      munsup=1.80d0		![fm^-3] range limite superior
      npoint=1201		!number of points

c      muninf=0.95d0		!potencial quim (k^2+ms^2)^1/2   [fm^-3] range limite inferior
c      munsup=2.60d0		![fm^-3] range limite superior
c      npoint=1501		!number of points
...
c     initializing variables
      ...
      x(2)=0.01d0            		! field vsigma
      x(3)=0.8d0                        ! field vomega
      x(4)=0.02d0                       ! filed vrho 
c
c---------------------------------------------------------------------

      do i=1, npoint
      mun=munsup-(i-1)*dmub		!decrescente
...
c	tornando o momento de fermi positivo 
      ...
      call eos(mue,kfnu,kfnd,vsigma,vomega,vrho,ener,press)       !the eos

      nbtd=nbt*(m/197.32d0)**3
      
c      write(2,10) nbtd/denss*0.153d0,(nl(j)/nbt,j=1,2),
c     &               (nb(j)/nbt,j=1,8)
      
c      write(50,*)nbtd/denss, (vomega-vsigma+
c     & (nb(2)*vrho-nb(1)*vrho)/(nb(1)+nb(2)))*939.d0 		!meson potentials

      ff1 = 0.333d0*(nb(3)+nb(4)+nb(5)+nb(6)+2*(nb(7)+nb(8)))  !fraçao de estranheza
    
c      write(51,*) nbtd/denss*0.153d0, ff1/nbt
...
c      write(14,*) nbtd, ne,nu,npu,nsm,nsp,nxm
...
c	Funcoes para broyden
      ...
      fvec(3)=frho
      fvec(4)=charge
c      fvec(5)=rnumber-nbt
c
      do i=1,4
         fvec1(i)=fvec(i)
      enddo
      
c      write(6,*)mun,npu,ne,nu
c      write(6,*)np,ne,nu
c      write(6,*)(fvec(i),i=1,4)
c      write(6,*)mue,vsigma,vomega,vrho
      return
      end
c-----------------------------------------------------------------------
...
c      write(*,*) rkfmu2, rkfmu**2,rkfp2,rkfp**2,rkfn2,rkfn**2
c      write(*,*) ener, enerf,enerbar,enerlep
c      write(*,*)
c      write(*,*) vsigma,vomega,vrho
